...
/** 打印信息 */
#define PRINT(p1)	{ std::ostringstream __FUNCTION____LINE__; __FUNCTION____LINE__<<p1<<std::endl; cout<<mirror::utils::StringUtils::Encoding(__FUNCTION____LINE__.str(), "UTF-8", "GBK")<<std::flush; }

#ifdef DEBUG
/** 输出调试信息 */
#define LOG_DEBUG(p1)	{ std::ostringstream __FUNCTION____LINE__; __FUNCTION____LINE__<<"  | C "<<mirror::utils::TimeUtils::MSTimeString()<<" | "<<p1; cout<<termcolor::green<<mirror::utils::StringUtils::Encoding(__FUNCTION____LINE__.str(), "UTF-8", "GBK")<<termcolor::reset<<" ... "<< __FUNCTION__<<"() "<<__FILE__<<"("<<__LINE__<<")"<<std::endl<<std::flush; }
#else
/** 输出调试信息 */
#define LOG_DEBUG(p1)
#endif
/** 输出系统信息 */
#define LOG_INFO(p1)	do{ \
